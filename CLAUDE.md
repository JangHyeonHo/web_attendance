...
멀티테넌트(SaaS) 웹 근태관리 시스템의 **백엔드**(Spring Boot). 프론트는 `frontend/`에 동거 중이며 별도 `frontend/CLAUDE.md`를 가진다(추후 레포 분리 예정 — 이 문서는 백엔드만 다룬다).

## 스택

- Java 21, Spring Boot 4.1.0, MyBatis(spring-boot-starter 4.0.1, 어노테이션 매퍼), MariaDB, Flyway
...
./mvnw spring-boot:run                # :9080 — 플러그인 설정으로 dev 프로파일 자동 적용
./mvnw test                           # 전체 테스트
...
- 목록 페이지네이션 응답은 `common/PageResponse`(items/page/size/totalCount/totalPages, 기본 20건·상한 100)

## 멀티테넌시·인가(핵심 규약)

- **tenantId는 항상 세션(`SessionUser`)에서만 취득** — 요청 파라미터의 테넌트 값 불신
- 테넌트 식별은 서브도메인/코드 병행(`TenantHostResolver`): `app.tenant.base-domain` 설정 시 `{label}.{base-domain}`의 label을 테넌트 코드로 해석(Host 헤더 기반 — 프록시가 Host를 보존해야 함), 미설정(기본)이면 코드 입력 방식만
...
- `SessionUser` 필드 변경 시 serialVersionUID 증가 = 전 유저 재로그인

## 화면·i18n(프론트와의 계약)

- **서버 주도 내비게이션**: 프론트는 URL 라우팅 없이 `POST /api/v1/navigation` 응답의 화면 코드로만 화면을 결정한다
- 화면 코드 정본은 `navigation/Screen.java` enum: `M###` 멤버 본인 업무 / `T###` 테넌트 관리 / `A###` 운영사 / `W###` 공통. 새 화면은 해당 접두사의 다음 번호. W003은 영구 결번
- 화면 텍스트는 DB `language_master`(window_id=화면 코드, `W999`=공통 라벨) — 프론트는 텍스트 사전을 갖지 않으므로 라벨 추가는 항상 이쪽 Flyway 마이그레이션으로
