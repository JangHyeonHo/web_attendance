...
- 정적 UI 텍스트는 `src/i18n/builtin.ts`의 내장 사전(3개 언어) 사용.
  **서버 언어 마스터에 같은 키가 등록되어 있으면 그 값이 우선**한다
  (관리자가 W005에 `ATTEND=출근하기`를 등록하면 버튼 문구가 바뀜 — E2E로 검증됨).
...
├── i18n/builtin.ts       # 내장 UI 텍스트(3개 언어) + t() 헬퍼
└── screens/
