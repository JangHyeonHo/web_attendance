...
public final class UserDtos {
    ...
    @Schema(description = "schema.signup-request")
    public record SignupRequest(
            @Schema(description = "schema.signup-request.email", example = "hong@example.com")
            @NotBlank(message = "{validation.email.required}")
            @jakarta.validation.constraints.Email(message = "{validation.email.format}")
            @Size(max = 100, message = "{validation.email.size}")
            String email,

            @Schema(description = "schema.signup-request.password", example = "Passw0rd!")
            ...
            @Schema(description = "schema.signup-request.depart-cd", example = "DEV01")
    ...
    public record UserResponse(
            ...
            @Schema(description = "schema.field.email", example = "hong@example.com") String email,
            @Schema(description = "schema.field.name", example = "홍길동") String name,
            @Schema(description = "schema.field.depart-cd", example = "DEV01") String departCd,
            @Schema(description = "schema.field.admin", example = "false") boolean admin) {
