# 프론트엔드(React) 점검 보고서 — 이력 문서(아카이브)

> **⚠ 이 문서는 v2.0.0 시점의 점검 기록이며, 이후 방향이 바뀌었다.**
> - §1·§5의 "표준 URL 라우팅 전환" 권고는 **미채택** — 소유자 결정으로 v1의 **서버 주도 화면전환(W코드)을
>   복원**했다(`POST /api/v1/navigation`, 배경은 patch-notes-2026-07.md **D7**).
> - 회원가입(Signup)과 `POST /api/v1/users`는 SaaS 전환(v2.1)에서 **폐지** — 멤버 등록은
>   고객사 관리자의 W009에서만(patch-notes-2026-07-saas.md **D11**).
> - 점검 대상이던 `front_react/`는 TS 재작성 시점에 삭제됐다. **현행 프론트 문서는 `frontend/README.md`.**
> - §2~§4·§6(구 코드 문제 분석·의존성 감사·빌드 실측)은 재작성 결정의 근거로 유효한 기록이라 남긴다.

- 대상: `front_react/web-attendance-front/` (React 17 + CRA, 2021년 작성, 약 800줄 — **v2.1에서 삭제됨**)
...
## 5. 리뉴얼 권장안 (골격·정리 항목은 채택, 라우팅·Signup은 미채택 — 상단 배너 참조)
