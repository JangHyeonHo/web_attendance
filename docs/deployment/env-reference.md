...
| `APP_CRYPTO_KEY` | `openssl rand -base64 32` 출력 | 민감 필드 AES-256-GCM 키. ★분실=복호화 불가, 백업과 분리 보관([11](11-backup-restore.md))★. dev 기본키로 암호화된 데이터는 운영 반입 금지 |
...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `APP_TENANT_BASE_DOMAIN` | (비움 = 꺼짐) | 테넌트 서브도메인 병행 방식([13](13-tenant-subdomain.md)). 와일드카드 DNS/TLS 준비 후에만 설정 |
...
- prod에서 기동 실패 시 로그의 `Caused by`에 어느 변수가 비었는지 나온다
  ([06](06-backend-service.md) 기동 확인 참조).
