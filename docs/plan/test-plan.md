...
| 마지막 관리자 보호 위반 | **409** + 코드 `LAST_ADMIN` | 검증 오류가 아닌 상태 충돌 |
...
| `SA` | SYSTEM_ADMIN (운영사, 무테넌트 또는 시스템 테넌트 소속) |
...
| ISO-07 | S | TA-A | `GET/PUT/DELETE /api/v1/tenant/members/{M-B.id}` (조회/수정/비활성) | 404 | ID 추측 공격 차단, 존재 비노출 |
| ISO-08 | S | TA-A | B 멤버의 출결 현황/월별 상세(관리자용 조회 API, `userId=M-B.id`) | 404 | 매퍼 2중 조건(`AND tenant_id=`)이 실 SQL에서 동작 |
...
| ROLE-12 | `PUT /api/v1/system/tenants/{id}` (정지/재개) | 401 | 403 | 403 | 200 |
...
| ROLE-15 | `PUT /api/v1/tenant/members/{id}` (role/status 변경) | 401 | 403 | 200 | **403** |
...
| ID | 레벨 | 입력 | 기대 |
|----|------|------|------|
| NAV-01 | U | MEMBER가 W004 요청 | 출결 화면 + `ADMIN_ONLY` (현행 유지) |
| NAV-02 | U | TENANT_ADMIN이 W004 요청 | 테넌트 관리(멤버 관리) 화면 |
| NAV-03 | U | SYSTEM_ADMIN이 W004/홈 요청 | 테넌트 목록 화면(W007) |
| NAV-04 | U | TENANT_ADMIN이 W007 요청 | 자기 홈으로 + 거부 사유 |
| NAV-05 | U | SYSTEM_ADMIN이 W005(출결)/W006(상세) 요청 | 자기 홈으로 + 거부 사유 (API뿐 아니라 화면 전개에서도 차단) |
...
| ID | 레벨 | 시나리오 | 기대 |
|----|------|----------|------|
| ADM-01 | U+S | 테넌트의 **유일한** TENANT_ADMIN을 MEMBER로 강등 | 409 `LAST_ADMIN` |
| ADM-02 | U+S | 유일한 TENANT_ADMIN을 DISABLED로 비활성 | 409 `LAST_ADMIN` |
...
대상: `spring-security-crypto` 기반 AES-256-GCM 유틸(가칭 `FieldCipher`)과 마스킹 유틸(가칭 `Masking`). 전부 순수 단위(U) — DB·컨텍스트 불요.
...
| CRY-02 | U | **키 버전 프리픽스**: 암호문이 `{v1}` 프리픽스 + Base64 페이로드 형식 | 프리픽스 파싱 함수 단위 검증. 프리픽스 없는 입력은 명시적 예외(레거시 평문 오인 방지) |
| CRY-03 | U | 키 로테이션: v1 키로 만든 암호문을 v2가 현행 키인 키맵(`{v1:…, v2:…}`)에서 복호화 | 성공. 신규 암호화는 항상 최신 버전 프리픽스 |
| CRY-04 | U | 무결성(GCM 태그): 암호문 중간 1바이트 변조 후 복호화 | 예외 발생(silent corruption 없음) |
| CRY-05 | U | IV 랜덤성: 같은 평문 2회 암호화 | 서로 다른 암호문(IV 재사용 없음 — GCM에서 IV 재사용은 치명적) |
| CRY-06 | S | 키 미주입 기동: `ENCRYPT_KEY` 환경변수 없이 앱 기동 | **기동 실패(fail-fast)** — 평문 저장으로 조용히 진행되지 않음 |
...
| MSK-05 | U | toString: `TenantProfile`/`TenantBilling` 도메인 객체의 `toString()` 출력 | 사업자번호·전화·빌링키 원문/암호문이 **미포함** (로그 유출 방지, §6-1 원칙 3) |
...
| `NavigationServiceTest` (anonymous, loggedInUser, admin, logout, unknownScreen) | 5 | **C 재설계** | `SessionUser(id, email, name, boolean)` → `(id, tenantId, email, name, role)` 전환으로 픽스처 전면 교체. `anonymous`: W003(SIGNUP) 기대값이 가입 폐기로 변경. `loggedInUser` → MEMBER 케이스로 개명. `admin` → TENANT_ADMIN/SYSTEM_ADMIN **2건으로 분리**(W004 분기 상이). §2-2 NAV-01~06이 신규 추가분 |
| `MessagesTest` (resolvesPerLocale, formatsArguments, fallsBackToDefault, bundlesHaveSameKeys) | 4 | **A** | 없음. 신규 메시지 키(테넌트/멤버/마스킹/LAST_ADMIN 등)를 3언어 동시 추가하지 않으면 `bundlesHaveSameKeys`가 **자동으로 실패** — 이 테스트가 신규 기능의 번역 게이트 역할 |
...
```
사전: docker compose up -d && ./mvnw spring-boot:run (ENCRYPT_KEY 주입) && npm run dev
계정: SA(시드), 이후 단계에서 생성되는 TA-A, TA-B, hong@example.com(A/B 각각)
```

| # | 단계 | 조작 (Playwright 수준) | 단언 |
|---|------|------------------------|------|
| 1 | 랜딩 | `page.goto('/')` | W000 인덱스 렌더링, 헤더·언어 스위처 표시 (URL 라우팅 없음 — 이후 전 단계 URL 불변) |
| 2 | 로그인 화면 | 로그인 진입 | **회사 코드 입력 필드 존재**. 가입 링크(W003) 부재 |
| 3 | SA 로그인 | 코드 없이 또는 시스템 코드로 SA 크리덴셜 입력 | **테넌트 목록 화면(W007)** 으로 전개(출결 화면 아님) |
...
| 7 | TA-A 로그인 | 로그아웃 → `{ACME, ta-a@…, 초기비번}` | 멤버 관리 화면으로 전개 |
| 8 | 멤버 등록(A) | `hong@example.com` / 비번 `pwA` 등록 | 멤버 목록에 1건 |
| 9 | 멤버 로그인(A) | 로그아웃 → `{ACME, hong@…, pwA}` | 출결 화면(W005), 상태 "출근 대기" |
| 10 | 출결(A) | 출근 버튼 → 위치 허용 → 체크 → 확정 | 상태 "출근 중" + 성공 메시지. 월별 상세(W006)에 오늘 출근 시각 표시 |
| 11 | 멤버 등록(B) | 로그아웃 → TA-B 로그인 → **같은 이메일** `hong@example.com` / 비번 `pwB` 등록 | 등록 성공(테넌트 내 유니크 — 이메일 중복 오류가 나면 실패) |
...
기존 12단계 시나리오를 그대로 유지하되 변경점 2개만 반영: ① 로그인 폼에 회사 코드(`DEFAULT` 테넌트) 입력 ② 가입 화면(W003) 단계는 "TENANT_ADMIN의 멤버 등록"으로 대체. 그 외 단계(출결 체크→확정, 월별 상세, 언어 전환, 관리자 언어 마스터)는 **단언 무변경**으로 통과해야 한다.
...
| navigation 분기 NAV-01~06 | ● | — | #3,6,7 | 결정 규칙은 순수 함수(D7 설계 그대로) |
...
| 레벨 | 건수 | 내역 |
|------|------|------|
| 단위(U) | **69** | 기존 35(무변경 12 + 수정 18 + 재설계 5) + 신규 34 = ISO-14×3, LGN 5, ROLE 인터셉터 매트릭스 15셀분 1식(15케이스 상당→집계는 8메소드), NAV 6, ADM 5, CRY 6, MSK 6 |
| 스모크(S) | **약 42 항목** | ISO 13 + LGN 7 + ROLE 15 + ADM 3 + CRY-06 + MSK 4 + REG-04 (전 API 재실행 1식) |
