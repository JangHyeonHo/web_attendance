...
<div className="landing">
  <section className="hero">            // 히어로
    <h1>{t('INDEX_TITLE')}</h1>          // 기존 키 재사용
    <p>{t('LANDING_HERO_SUB')}</p>
    {userName
      ? <button className="primary" onClick={→ navigate('W005')}>{t('LANDING_GO_ATTEND')}</button>
      : <a className="cta" href={`mailto:${t('CONTACT_EMAIL')}?subject=${encodeURIComponent(t('LANDING_CTA'))}`}>
          {t('LANDING_CTA')}
        </a>}
    <p className="muted">{t('LANDING_CTA_SUB')}</p>
  </section>
  <section className="features">        // 기능 소개 4종 (카드 그리드)
    {4 × <div className="feature-card"><h3>{t('FEATURE_*_TITLE')}</h3><p>{t('FEATURE_*_DESC')}</p></div>}
  </section>
</div>
```
...
| 섹션 | 내용 | 키(W000) |
|------|------|----------|
| 히어로 | 제품명 + 한 줄 소개 + CTA | `INDEX_TITLE`(재사용) `LANDING_HERO_SUB` `LANDING_CTA` `LANDING_CTA_SUB` `LANDING_GO_ATTEND` `CONTACT_EMAIL` |
| 기능 1 | 위치기반 출결(체크→확정 2단계, 좌표 기록) | `FEATURE_GEO_TITLE` `FEATURE_GEO_DESC` |
| 기능 2 | 다국어(한/영/일, DB 언어 마스터 즉시 반영) | `FEATURE_I18N_TITLE` `FEATURE_I18N_DESC` |
| 기능 3 | 월별 리포트(스케쥴 대비 실적, 휴일 반영) | `FEATURE_REPORT_TITLE` `FEATURE_REPORT_DESC` |
| 기능 4 | 기업별 격리(테넌트 단위 독립 유저·데이터) | `FEATURE_ISOLATION_TITLE` `FEATURE_ISOLATION_DESC` |
| CTA | **도입 문의 = mailto 링크** (폼/백엔드 없음) | 위 히어로 키에 포함 |

- **mailto 주소도 언어 마스터 키(`CONTACT_EMAIL`, 3언어 동일 값)로 관리**한다. "UI 텍스트는 DB
  단일 출처" 원칙의 연장으로, 문의 주소 변경을 배포 없이 관리자 화면에서 처리할 수 있다.
- 로그인 상태로 W000이 렌더될 일은 원칙적으로 없으나(서버가 홈으로 전개), 방어적으로
  `userName` 존재 시 CTA를 "출결 화면으로" 버튼으로 대체한다.
- 기존 `INDEX_SUB` 키는 폐기(V5에서 삭제, §7).
...
export interface LoginResponse {
  ...
  email: string
  name: string
  role: Role           // admin: boolean → role
  tenantName: string | null   // 헤더 뱃지용 (SYSTEM_ADMIN은 null)
}
```
...
- **마지막 관리자 보호 에러 표시**: 유일한 TENANT_ADMIN을 강등/비활성하려 하면 서버가 409 +
  `code: 'LAST_ADMIN_PROTECTED'`(메시지는 서버가 세션 언어로 조립)를 반환한다. 프론트는
...
| `LoginResponse` | `admin: boolean` → `role: Role`, `tenantName: string | null` 추가 |
| `SignupRequest` | **삭제** |
...
export interface TenantProfileResponse {        // 조회는 마스킹 필드만
...
에러 계약 추가: `ErrorResponse.code`에 `'LAST_ADMIN_PROTECTED'`(409) 등장 — 타입은 string
그대로(코드 유니온은 만들지 않음, 현행 컨벤션 유지).
...
export const systemTenantApi = {
  ...
  updateStatus: (tenantId: number, request: TenantStatusUpdateRequest) =>
    post<TenantSummary>(`/api/v1/system/tenants/${tenantId}/status`, request),
  ...
  upsertProfile: (tenantId: number, request: TenantProfileUpsertRequest) =>
    post<TenantProfileResponse>(`/api/v1/system/tenants/${tenantId}/profile`, request),
  ...
  upsertBilling: (tenantId: number, request: TenantBillingUpsertRequest) =>
    post<TenantBillingResponse>(`/api/v1/system/tenants/${tenantId}/billing`, request),
}
...
export const tenantMemberApi = {
  ...
  updateStatus: (userId: number, request: MemberStatusUpdateRequest) =>
    post<MemberSummary>(`/api/v1/tenant/members/${userId}/status`, request),
  updateRole: (userId: number, request: MemberRoleUpdateRequest) =>
    post<MemberSummary>(`/api/v1/tenant/members/${userId}/role`, request),
}
```

- **삭제**: `userApi`(signup 단일 함수만 있던 객체) 통째 제거.
- 변경 없음: `navigationApi`, `attendanceApi`, `languageApi`, `client.ts`
  (401 처리·에러 매핑 그대로. 멀티테넌시는 세션 스코프라 클라이언트 계층 무변경 —
...
| 언어 마스터 | V5에서 `window_id='W003'` 전 행 + `W999/SIGNUP` 삭제(§7). `NAME`·`DEPART`는 W009로 재배치 |
...
`V5__seed_tenancy_ui_texts.sql`. V3와 동일하게 `INSERT IGNORE`(운영자 수정 존중).
**선행 DELETE**: `W003` 전 행, `W999/SIGNUP`, `W000/INDEX_SUB` (폐기 키 정리 — 관리 화면 노이즈 방지).
...
### W000 랜딩 (신규 13키 / `INDEX_TITLE` 재사용, `INDEX_SUB` 삭제)

| 키 | KOR | ENG | JPN |
|----|-----|-----|-----|
| LANDING_HERO_SUB | 위치 기반 출결과 월별 리포트를 갖춘 기업용 근태 관리 SaaS | Location-based attendance and monthly reports for your company | 位置情報打刻と月次レポートを備えた企業向け勤怠管理SaaS |
| LANDING_CTA | 도입 문의하기 | Contact sales | 導入のお問い合わせ |
| LANDING_CTA_SUB | 이메일을 보내주시면 담당자가 안내해 드립니다. | Send us an email and we will get back to you. | メールをお送りいただければ担当者がご案内します。 |
| CONTACT_EMAIL | sales@webatt.example | sales@webatt.example | sales@webatt.example |
| LANDING_GO_ATTEND | 출결 화면으로 | Go to attendance | 勤怠画面へ |
| FEATURE_GEO_TITLE | 위치 기반 출결 | Location-based stamping | 位置情報打刻 |
| FEATURE_GEO_DESC | 출근·퇴근·휴식을 현재 위치와 함께 2단계 확인으로 기록합니다. | Clock in/out with your location, confirmed in two steps. | 出退勤・休憩を現在地とともに2段階確認で記録します。 |
| FEATURE_I18N_TITLE | 3개 언어 지원 | Three languages | 3言語対応 |
| FEATURE_I18N_DESC | 한국어·영어·일본어 화면과 메시지를 즉시 전환합니다. | Switch the entire UI between Korean, English and Japanese. | 韓国語・英語・日本語のUIとメッセージを即時切替。 |
| FEATURE_REPORT_TITLE | 월별 리포트 | Monthly reports | 月次レポート |
| FEATURE_REPORT_DESC | 스케쥴 대비 실적과 휴일을 한 표에서 확인합니다. | Compare schedules and actual stamps in one monthly view. | スケジュールと実績・休日をひとつの表で確認。 |
| FEATURE_ISOLATION_TITLE | 기업별 격리 | Isolated per company | 企業ごとに分離 |
| FEATURE_ISOLATION_DESC | 회사마다 독립된 계정과 데이터로 안전하게 운영합니다. | Each company gets its own isolated users and data. | 会社ごとに独立したアカウントとデータで安全に運用。 |
...
합계: **신규 74키 × 3언어 = 222행**, 삭제 4키(W003 3키 + W999/SIGNUP) + W000/INDEX_SUB.
(INITIAL_PWD 계열은 W007/W009에 의도적 중복 — 화면 단위 텍스트 배치 컨벤션 유지.
기존 W005/W006의 ATTDETAILS 중복과 같은 원칙.)

### 서버 메시지(언어 마스터 아님 — 참고)

마지막 관리자 보호(409 LAST_ADMIN_PROTECTED), 로그인 실패, 검증 에러 등 **서버 생성 메시지는
...
| T4 | **랜딩 표시** | 비로그인 접속 → W000 랜딩: 히어로 타이틀, 기능 카드 4종, CTA가 `mailto:` href(언어 마스터의 CONTACT_EMAIL)로 렌더. 헤더에 **SIGNUP 버튼 없음**. 언어 3종 전환 시 랜딩 텍스트 전량 교체(키 노출 없음). `navigate('W003')` 강제 호출 → W000 폴백. 로그인 상태에서 W000 요청 → 각자 홈으로 전개 |
...
| 수정 | `App.tsx`(스위치·헤더 role 분기), `app/AppContext.tsx`(role 상태), `api/types.ts`, `api/endpoints.ts`, `screens/LoginScreen.tsx`, `app.css` |
| 삭제 | `screens/IndexScreen.tsx`(LandingScreen으로 대체), `screens/SignupScreen.tsx` |
| 무변경 | `api/client.ts`, `i18n/lang.ts`, `main.tsx`, `AttendanceScreen.tsx`, `DetailsScreen.tsx`, `AdminScreen.tsx` |
| 시드 | `V5__seed_tenancy_ui_texts.sql` — 신규 74키×3언어 + 폐기 키 삭제(§7) |
